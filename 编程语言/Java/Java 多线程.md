...
这里定义和线程相关的另一个术语  
- 进程：进程是操作系统运行程序的最小单元，一个进程包括由操作系统分配的内存空间，包含一个或多个线程。
- 进程是最小的资源分配单元
- 线程是最小的调度单元，一个线程不能独立的存在，它必须是进程的一部分。一个进程一直运行，直到所有的非守护线程都结束运行后才能结束，线程共享进程的资源。
        ...
        () -> {
          try {
            while (index <= MAX_VALUE) {
              synchronized (obj) {
                if (index % 2 == 0) {
                  System.out.print(index == MAX_VALUE ? index:index + "->");
                  index++;
                  obj.notify();
                } else {
                  obj.wait();
